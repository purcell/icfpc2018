...
spec =
  describe "performCommand" $ do
    describe "Halt" $ it "is unimplemented" $ True === True
    describe "Wait" $
      it "returns the state with the wait command appended to the trace" $
      performCommand (initialBotId, Cmd.Wait) initialState `shouldBe`
      Just initialState {trace = [Cmd.Wait]}
    describe "FlipHarmonics" $
      it "flips dem harmonics" $
      harmonics <$>
      performCommand (initialBotId, Cmd.FlipHarmonics) initialState `shouldBe`
      Just High
    describe "SMove" $ do
      ...
      it "moves the bot by the specified vector" $ do
        let expectedBot =
              initialBot {coord = Coordinate {cx = 5, cy = 0, cz = 0}}
        bots <$>
          maybeState `shouldBe`
    ...
    describe "LMove" $ do
      let sld1 = SLD VectorDiff {dx = 0, dy = 3, dz = 0}
          ...
          maybeState =
            performCommand (initialBotId, Cmd.LMove sld1 sld2) initialState
      it "moves the bot by the specified vector" $ do
        let expectedBot =
              initialBot {coord = Coordinate {cx = 0, cy = 3, cz = 7}}
        bots <$>
          maybeState `shouldBe`
          (Just $ Map.fromList [(initialBotId, expectedBot)])
      it "adjusts the energy to reflect that a move was made" $
    ...
    describe "Fill" $ do
      ...
      it "fills the voxel at the specified vector" $ do
        ...
        ((flip isFilled) expectedCoord . matrix) <$>
          maybeState `shouldBe` Just True
      it
        "does not fill the voxel at the specified vector if it is not filled in the target" $ do
        let ncd = NCD VectorDiff {dx = 1, dy = 0, dz = 2}
            expectedCoord = Coordinate {cx = 1, cy = 0, cz = 2}
            maybeState =
              performCommand (initialBotId, Cmd.Fill ncd) initialState
        ((flip isFilled) expectedCoord . matrix) <$>
          maybeState `shouldBe` Nothing
      ...
      it "adjusts the energy by 6 when the specified voxel was Full" $
        energy <$>
        (maybeState >>= performCommand (initialBotId, Cmd.Fill ncd)) `shouldBe`
        Just 18 -- 12 for the first Fill, 6 for the second
